...
Welcome. This is a self-paced, evergreen course, which means there is no cohort to wait for and no start date to miss. You enroll whenever you're ready, and you move at whatever pace fits your life. Some learners finish a module in an evening; others spread it across a fortnight. Both are fine. The material doesn't expire, and neither does your access to it.

Here's what a typical module looks like and how everything fits together.

## Read, then build

Each module begins with a lesson. Read it first — it introduces the ideas in plain language, motivates why the method matters for real biological data, and sets up the reasoning you'll need before you touch any code. Skimming works less well here than in most places, because running the code yourself assumes you've absorbed the conceptual thread.

Once the lesson clicks, you run it. The lesson *is* a notebook: the same page you just read opens in **Google Colab** with a single click, so you can run and modify every example yourself. There is nothing to install, no environment to configure, no dependency conflicts to debug at midnight. Colab hosts the notebook in your browser, and the deep-learning module's models are deliberately small — they train in seconds on Colab's free CPU tier, and the code will use a GPU automatically if Colab happens to offer one. Either way you train real models without owning a GPU or paying for cloud time. You write and run code in the very place you read the explanation of what it does.
...
**Quantified claims with named limitations.** Every result you report is a precise claim — carrying the numbers that back it — next to a list of the specific things that could undermine it. "It works" is not a finding. Real science is quantified uncertainty: a result is genuinely useful when you state exactly what you measured and how well, and dangerous when you present it as more certain than the evidence allows. We deliberately *don't* reduce this to a one-word confidence label. Assigning "low / moderate / high" is subjective — the right level depends on the problem and on who is reasoning about it — and it tempts everyone to argue the label instead of the substance. Put the uncertainty where it is concrete instead: in the claim itself (effect sizes, intervals, held-out scores) and in the named limitations that tell others where to push. The `ddm4bio.interpret` helpers make this the easy path — a structured claim-and-limitations block — so the honest version is also the convenient version.

**Ground-truth validation, then honest real-data claims.** Before you report a method on real biological data, you show it recovers a *known* answer planted in synthetic data. But be precise about what that earns: the synthetic result **verifies the implementation and its recovery under stated assumptions** — necessary, not sufficient. The real-data conclusion still requires data-specific QC, sensitivity analysis, and external or biological validation. If a method can't recover a signal you planted yourself, it certainly can't be trusted to find one you didn't — but recovering the planted signal does not, by itself, license the real-data answer. This is why the library ships with ground-truth data generators: validation is the first step, not the last. For exactly when this course uses real versus synthetic data — and why both belong — see **[Real Data and Synthetic Ground Truth](data-and-ground-truth.md)**.

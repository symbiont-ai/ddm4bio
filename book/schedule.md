...
| **W8** | Deep learning for representations and dynamics: linear and nonlinear autoencoders (a linear autoencoder is exactly PCA), a convolutional net on blood-cell images, and shallow sensing-and-reconstruction (SHRED) architectures. | Learning a nonlinear latent that follows a curved manifold, and reconstructing a tissue's full electrical field from a handful of electrodes (as in electrocardiographic imaging). | Synthetic manifold & field fixtures · Wisconsin (WDBC) · **BloodMNIST** blood-cell images |
